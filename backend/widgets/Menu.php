...
class Menu extends \yii\widgets\Menu
{
    ...
    protected function renderItem($item)
    {
        ...
        if (!ArrayHelper::getValue($item, 'badgeOptions.class')) {
            $bgClass = isset($item['badgeBgClass']) ? $item['badgeBgClass'] : $this->badgeBgClass;
            $item['badgeOptions']['class'] = $this->badgeClass . ' ' . $bgClass;
        }
        ...
        if (isset($item['url'])) {
            return $this->prepareLinkItem($item);
        } else {
            return $this->prepareItem($item);
        }
    }

    private function prepareLinkItem($item)
    {
